...
func getReports(s *store.Store, urls []string) source.Source[*downloader.Report] {
	src := source.FromSlice(urls)

	return task.Connect(
		src.AsTask(),
		pipe.Map(func(ctx context.Context, url string) (*downloader.Report, error) {
...
func runReport(path string, urls []string) (*flow.Flow, error) {
	...
	sin := task.Connect(
		reps.AsTask(),
		sink.ElementWise(func(ctx context.Context, rep *downloader.Report) error {
